...
const MultipleSelect = ({name, options}) => {
    /** 
     * Custom select that supports multiple selection
     * @param {name} name that will be referenced in form
            ...
            <div className="space-y-3">
                {options.map((option) => (
                    <label key={option.value} className={`flex items-center p-3 rounded-lg transition-all duration-200 ${selectedOptions.includes(option.value) ? "bg-gray-50 border border-gray-200" : "bg-gray-50 hover:bg-gray-100 border border-gray-200"}`}>
                        <input name={name} type="checkbox" value={option.value} checked={selectedOptions.includes(option.value)} onChange={handleCheckboxChange} className="w-5 h-5 text-gray-400 border-gray-300 rounded focus:ring-gray-950"/>
